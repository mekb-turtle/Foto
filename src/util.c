...
SDL_Rect get_fit_mode(SDL_Point image_size_, SDL_Point window_size_) {
	...
	if (image_size.x / image_size.y > window_size.x / window_size.y) {
		...
		return (SDL_Rect) {
				.x = 0, .y = (int) ((window_size.y * 0.5f) - (image_size.y * scale * 0.5f)),
				.w = window_size_.x, .h = (int) (scale * image_size.y)
		};
	} else {
		scale = window_size.y / image_size.y;
		return (SDL_Rect) {
				.x = (int) ((window_size.x * 0.5f) - (image_size.x * scale * 0.5f)), .y = 0,
				.w = (int) (scale * image_size.x), .h = window_size_.y
		};
	}
}
